...
export function CustomTableAppointmentsArtist() {
  ...
  // Extraer la informacion de usuario del almacen
  const myPassport = useSelector(getUserData);
  const token = myPassport.token;

  // Traer todas las citas del artista logeado
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        let perPage = 10;
        const res = await getAppointmentsByLogedArtists(token,currentPage, perPage);
        console.log(res);
        setAppointments(res.data.appointments__artist);
...
export default CustomTableAppointmentsArtist;
